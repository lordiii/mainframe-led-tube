...
LED_Ring *LED_RINGS = new LED_Ring[LED_TOTAL_RINGS]{};
LED_Pixel *LEDS = new LED_Pixel[LED_TOTAL_AMOUNT]{};

// LED Render Task
IntervalTimer *renderTask = new IntervalTimer();
...
void LED_render() {
    ...
    if (state->current == nullptr) return;
    ...
    if ((!state->halt && state->slowRate == 0) || state->singleStep ||
        ...
        if (state->current != nullptr) {
            updated = state->current->callback(state->data, delta);
        }
